...
string c = "-c"; string o = "-o"; string d = "-d"; string q = "-q"; string i = "-i"; string x = "-x"; string imd = "--init_distance";
...
void InputParser::checkArgs(){
	...
	if(!cmdOptionExists(q)){
		cout<<yellow<<"No se ha pasado el argumento '-q' con la ruta del archivo con los bundles de los qrs."<<def<<endl;
		qrFlag = false;
	}else qrFlag = true;
...
InputReader::InputReader(InputParser* input){
	...
	if(input->qrFlag) parseQRFile(input->getCmdOption(q));
