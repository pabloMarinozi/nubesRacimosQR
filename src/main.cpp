...
class InputParser;
void scaleMap(InputParser* inputParser, MapManager* mpMapManager, InputReader* mpInputReader);
void fillMap(InitializationStrategy* mpInitializationStrategy, int numFrames,
	InputParser* inputParser, MapManager* mpMapManager, InputReader* mpInputReader);
...
int main(int argc, char **argv) {
	...
	if(!inputParser->calibFlag || !inputParser->outputFlag){
		cout<<endl<<red<<"LOS ARGUMENTOS '-c' y '-o' SON OBLIGATORIOS."<<endl;
		return 0;
	}
	if(!inputParser->qrFlag && !inputParser->detectionsFlag){
		cout<<endl<<red<<"AL MENOS UNO DE LOS ARGUMENTOS '-q' y '-d' ES NECESARIO."<<endl;
		return 0;
	} 
	...
	scaleMap(inputParser, mpMapManager, mpInputReader);


	//Iterar sobre los frames para procesar las observaciones
	fillMap(mpInitializationStrategy, numFrames, inputParser, mpMapManager, mpInputReader);
	...
	if(inputParser->imagesFlag) mpOutputWriter->graficarReproyecciones(mpInputReader, graficarNoVisibles, inputParser->scaleFlag);
...
void scaleMap(InputParser* inputParser, MapManager* mpMapManager, InputReader* mpInputReader){
	Color::Modifier yellow(Color::FG_YELLOW);
	Color::Modifier def(Color::FG_DEFAULT); 
	if(inputParser->scaleFlag){
		if(inputParser->qrFlag){
			string x = "-x";
			float distancia_calibracion = stof(inputParser->getCmdOption(x));
		...
		}else cout<<endl<<yellow<<"EL MAPA NO SERÁ ESCALADO PORQUE NO SE HA PASADO EL ARGUMENTO '-q'."<<def<<endl;
	...
	InputParser* inputParser, MapManager* mpMapManager, InputReader* mpInputReader)
{
	...
	for (auto i : kfRestantes) {
		...
		// if (cal_1>=0 & cal_2>=0) cout<<mpMapManager->GetDistanceCal1Cal2(cal_1,cal_2)<<endl;
	}
}
